...
let cNames = ['SEE 100', 'SEE 101W', 'SEE 110', 'SEE 111', 'SEE 221', 'SEE 222', 'SEE 224', 'SEE 225', 'SEE 230', 'SEE 231',
  ...
  'SEE 462', 'SEE 463', 'PHYS 140', 'PHYS 141', 'CHEM 121', 'CMPT 130', 'CMPT 135', 'BUS 238', 'REM 321', '', 'GEOG 324',
...
let BUSBG = [214, 214, 214]; //All Grey Buttons
let COOPBG = [252, 134, 144];
let ELECBG = [173, 150, 255];

//Button Border Colors (Currently matching backgrounds)
let SEEbdr = [0, 0, 0];
...
let BUSbdr = [214, 214, 214]; //All Grey Buttons
let COOPbdr = [252, 134, 144];
let ELECbdr = [173, 150, 255];

//Border Default Color Array (applies button border colors)
let bdca = [];
for (let x = 0; x < 4; x++) {
  bdca[x] = SEE100BG;
}
...
//Background Default Color Array (applies button background colors)
let bgdca = [];
for (let x = 0; x < 4; x++) {
  bgdca[x] = SEE100BG;
}
...
//Button Height
let bsH = 40;
//Button Height with Padding
let bspH = bsH + 2;
//Spacing Buffers (Y unused)
let spacingX = 15;
let spacingY = 40;
...
function setup() {
  ...
  //Summer Semesters (last line is spring year 4)
  ...
  //Label Sideways Transformations
  push();
  ...
  buttons[39].style('opacity', 0.4);
  buttons[39].style('cursor', 'not-allowed');
  for (let x = 51; x < 54; x++) {
  ...
  //buttons[39].mousePressed(functions39);  GEOG 323 Removed
